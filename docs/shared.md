...
结果按 Zig 值语义复制。指针/slice 仍引用原对象，不会深拷贝，也没有自动调用 `deinit` 的 RAII 行为。外部资源的生命周期仍由调用方管理。与 C++ stdexec 常用的 const-reference 共享完成形式相比，这是明确的 Zig API 取舍。
...
共享上游的内部 receiver 提供 shared owner 创建时传入的 allocator。不同订阅可以有不同 allocator；共享执行不继承任一订阅的 allocator，以免上游或缓存结果依赖已结束的订阅环境。用户结果仍采用值复制且没有隐式析构，详见 [allocator 设计](allocators.md)。
