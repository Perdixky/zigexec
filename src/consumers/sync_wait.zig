const c = @import("../execution/protocol.zig");
const RunLoop = @import("../schedulers/run_loop.zig").RunLoop;
const StartScheduler = @import("../schedulers/start_scheduler.zig");

/// Wait for completion while driving a caller-thread run loop.
/// Supplies getStartScheduler when absent, so asynchronous join can resume here.
pub fn syncWait(sender: anytype, env: anytype) anyerror!?@TypeOf(sender).Values {
    const E = @import("../execution/environment.zig").Normalize(@TypeOf(env));
    const Values = @TypeOf(sender).Values;
    const State = struct {
        loop: *RunLoop,
        result: c.CompletionRef(Values) = undefined,
        env: E,
        pub fn getEnv(self: *@This()) E {
            return self.env;
        }
        fn completeOwnership(self: *@This()) void {
            self.loop.finish();
        }
        pub fn setValue(self: *@This(), values: *const Values) void {
            defer self.completeOwnership();
            self.result = .{ .value = values };
        }
        pub fn setError(self: *@This(), err: anyerror) void {
            defer self.completeOwnership();
            self.result = .{ .err = err };
        }
        pub fn setStopped(self: *@This()) void {
            defer self.completeOwnership();
            self.result = .stopped;
        }
    };
    var loop: RunLoop = .{};
    const scheduler = loop.getScheduler();
    var wait_env = @import("../execution/environment.zig").normalize(env);
    if (wait_env.start_scheduler == null) wait_env.start_scheduler = StartScheduler.init(&scheduler);
    var state: State = .{ .loop = &loop, .env = wait_env };
    var operation: @import("../execution/connect.zig").Connection(@TypeOf(sender), *State) = undefined;
    c.connectInto(&operation, sender, &state);
    operation.start();
    loop.run();
    return switch (state.result) {
        .value => |values| values.*,
        .err => |err| err,
        .stopped => null,
    };
}
